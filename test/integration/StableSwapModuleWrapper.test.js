...
const setup = async () => {
  ...
  await USDT.approve(stableSwapModuleWrapper.address, ethers.constants.MaxUint256, { gasLimit: 1000000 });
  await fathomStablecoin.approve(stableSwapModuleWrapper.address, ethers.constants.MaxUint256, { gasLimit: 1000000 });
...
xdescribe("StableSwapModuleWrapper", () => {
  ...
  describe("#ShouldDepositTokens", async () => {
    ...
    context("Should not deposit tokens and revert for not whitelisted people", () => {
      it("Should deposit", async () => {
        await USDT.approve(stableSwapModuleWrapper.address, ethers.constants.MaxUint256, { gasLimit: 1000000, from: accounts[2] });
        await fathomStablecoin.approve(stableSwapModuleWrapper.address, ethers.constants.MaxUint256, { gasLimit: 1000000, from: accounts[2] });
        ...
        await expect(stableSwapModuleWrapper.depositTokens(TO_DEPOSIT, { gasLimit: 1000000, from: accounts[2] })).to.be.revertedWith(
  ...
  describe("#ShouldDepositTokensAndSwap", async () => {
    context("Should let whitelisted people deposit Tokens and then swap", () => {
      it("Should deposit", async () => {
        ...
        await fathomStablecoin.mint(accounts[2], TO_DEPOSIT, { gasLimit: 1000000 });
        await stableSwapModuleWrapper.depositTokens(TO_DEPOSIT, { gasLimit: 1000000 });
        await stableSwapModule.swapStablecoinToToken(accounts[2], ONE_PERCENT_OF_TOTAL_DEPOSIT_SINGLE_SWAP_FIT, { gasLimit: 1000000 });
        await stableSwapModule.swapStablecoinToToken(accounts[2], ONE_PERCENT_OF_TOTAL_DEPOSIT_SINGLE_SWAP_FIT, { gasLimit: 1000000 });
      });
    });
  });
  ...
  describe("#withdrawTokens from Stableswap with stableswapWrapper", async () => {
    context("Should withdraw tokens from stableswap", () => {
      it("Should withdraw", async () => {
        await stableSwapModuleWrapper.withdrawTokens(WeiPerWad, {
    ...
    context("Should withdraw tokens from stableswap as per the ratio with swap stablecoin to token", () => {
      it("Should withdraw", async () => {
        ...
        await stableSwapModuleWrapper.withdrawTokens(amountToWithdraw, {
          from: DeployerAddress,
          gasLimit: 8000000,
        });
    ...
    context("Should withdraw tokens from stableswap as per the ratio with swap token to stablecoin", () => {
      it("Should withdraw", async () => {
        ...
        const amountToWithdraw = WeiPerWad.mul(200);
        await stableSwapModuleWrapper.withdrawTokens(WeiPerWad.mul(200), {
          from: DeployerAddress,
          gasLimit: 8000000,
        });
  ...
  describe("#decentralizedState", async () => {
    ...
    context("set decentralized state and deposit tokens should succeed then, set decentralized state as false and should fail", async () => {
      it("Should succeed", async () => {
        ...
        await expect(stableSwapModuleWrapper.depositTokens(TO_DEPOSIT, { from: accounts[1] })).to.be.revertedWith("user-not-whitelisted");
      });
    });
  });
  ...
  describe("#depositTokens", async () => {
    context("deposit USDT and FXD as not whiteListed account", async () => {
      it("should fail", async () => {
        ...
        await expect(stableSwapModuleWrapper.depositTokens(TO_DEPOSIT, { from: accounts[1], gasLimit: 8000000 })).to.be.revertedWith(
          "user-not-whitelisted"
        );
      });
    });
