...
describe("CollateralPoolConfig", () => {
  ...
  beforeEach(async () => {
    ...
    collateralPoolConfigAsAlice = collateralPoolConfig.connect(provider.getSigner(AliceAddress));

    mockedAccessControlConfig = await smock.fake("AccessControlConfig");
    mockedSimplePriceFeed = await smock.fake("SimplePriceFeed");
    mockedCollateralTokenAdapter = await smock.fake("TokenAdapter");

    mockedAccessControlConfig.PRICE_ORACLE_ROLE.returns(formatBytes32String("PRICE_ORACLE_ROLE"));
    mockedAccessControlConfig.BOOK_KEEPER_ROLE.returns(formatBytes32String("BOOK_KEEPER_ROLE"));
    ...
    // mockedAccessControlConfig.collateralPoolId.returns(COLLATERAL_POOL_ID)

    mockedSimplePriceFeed.peekPrice.returns([formatBytes32BigNumber(BigNumber.from("0")), true]);
    mockedCollateralTokenAdapter.decimals.returns(0);
    mockedCollateralTokenAdapter.collateralPoolId.returns(COLLATERAL_POOL_ID);
  ...
  describe("#initCollateralPool", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        ...
        mockedSimplePriceFeed.poolId.returns(COLLATERAL_POOL_ID);
        mockedSimplePriceFeed.isPriceOk.returns(true);
    ...
    context("price feed is not healthy", () => {
      it("should revert", async () => {
        mockedSimplePriceFeed.poolId.returns(COLLATERAL_POOL_ID);
        mockedSimplePriceFeed.isPriceOk.returns(false);
    ...
    context("wrong price feed pool", () => {
      it("should revert", async () => {
        mockedSimplePriceFeed.poolId.returns(formatBytes32String("GOLD"));
        mockedSimplePriceFeed.isPriceOk.returns(true);
    ...
    context("adapter with wrong id", () => {
      it("should revert", async () => {
        mockedSimplePriceFeed.poolId.returns(COLLATERAL_POOL_ID);
        mockedSimplePriceFeed.isPriceOk.returns(true);
        mockedCollateralTokenAdapter.collateralPoolId.returns(formatBytes32String("GOLD"));
    ...
    context("when parameters are valid", () => {
      it("should success", async () => {
        ...
        expect(await (await collateralPoolConfig.collateralPools(COLLATERAL_POOL_ID)).adapter).to.be.equal(mockedCollateralTokenAdapter.address);
      });
    });
  });
  describe("#setPriceWithSafetyMargin", () => {
    context("when the caller is not the RriceOracle Role", () => {
      it("should be revert", async () => {
        mockedAccessControlConfig.hasRole.returns(false);
        await expect(collateralPoolConfigAsAlice.setPriceWithSafetyMargin(COLLATERAL_POOL_ID, WeiPerRay)).to.be.revertedWith("!priceOracleRole");
      });
    });
    context("when parameters are valid", () => {
      it("should success", async () => {
  ...
  describe("#setDebtCeiling", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setDebtCeiling(COLLATERAL_POOL_ID, WeiPerRay)).to.be.revertedWith("!ownerRole");
      });
    });
  ...
  describe("#setDebtFloor", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setDebtFloor(COLLATERAL_POOL_ID, WeiPerRay)).to.be.revertedWith("!ownerRole");
      });
    });
  ...
  describe("#setPriceFeed", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setPriceFeed(COLLATERAL_POOL_ID, mockedSimplePriceFeed.address)).to.be.revertedWith("!ownerRole");
      });
    });
    context("when price feed is zero", () => {
      it("should revert", async () => {
        await expect(collateralPoolConfigAsAlice.setPriceFeed(COLLATERAL_POOL_ID, AddressZero)).to.be.revertedWith(
          "CollateralPoolConfig/zero-price-feed"
        );
      });
    });
    context("price feed for another pool", () => {
      it("should revert", async () => {
        ...
        mockedSimplePriceFeed.poolId.returns(poolId);
        await expect(collateralPoolConfigAsAlice.setPriceFeed(COLLATERAL_POOL_ID, mockedSimplePriceFeed.address)).to.be.revertedWith(
          "CollateralPoolConfig/wrong-price-feed-pool"
        );
      });
    });
    context("price feed is not healthy", () => {
      it("should revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setPriceFeed(COLLATERAL_POOL_ID, mockedSimplePriceFeed.address)).to.be.revertedWith(
          "CollateralPoolConfig/unhealthy-price-feed"
        );
      });
    });
    context("when parameters are valid", () => {
  ...
  describe("#setLiquidationRatio", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setLiquidationRatio(COLLATERAL_POOL_ID, WeiPerRay)).to.be.revertedWith("!ownerRole");
      });
    });
  ...
  describe("#setStabilityFeeRate", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setStabilityFeeRate(COLLATERAL_POOL_ID, WeiPerRay)).to.be.revertedWith("!ownerRole");
      });
    });
    context("when stability fee rate invalid", () => {
      it("should be revert", async () => {
  ...
  describe("#setAdapter", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setAdapter(COLLATERAL_POOL_ID, mockedCollateralTokenAdapter.address)).to.be.revertedWith(
          "!ownerRole"
        );
      });
    });
    context("when the parameters are invalid", () => {
      it("should be revert when adapter's input is address(0)", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setAdapter(COLLATERAL_POOL_ID, ethers.constants.AddressZero)).to.be.revertedWith(
          "CollateralPoolConfig/setAdapter-zero-address"
        );
      });
      it("should be revert when collateralPoolId is wrong)", async () => {
        mockedAccessControlConfig.hasRole.returns(true);

        await expect(collateralPoolConfigAsAlice.setAdapter(ethers.constants.HashZero, mockedCollateralTokenAdapter.address)).to.be.revertedWith(
          "CollateralPoolConfig/setAdapter-wrongPoolId"
        );
      });
    });
    context("when parameters are valid", () => {
  ...
  describe("#setCloseFactorBps", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        mockedAccessControlConfig.hasRole.returns(false);
        await expect(collateralPoolConfigAsAlice.setCloseFactorBps(COLLATERAL_POOL_ID, CLOSE_FACTOR_BPS)).to.be.revertedWith("!ownerRole");
      });
    });
    context("when close factor bps is more than 10000", () => {
      it("should be revert", async () => {
        await expect(collateralPoolConfig.setCloseFactorBps(COLLATERAL_POOL_ID, BigNumber.from(20000))).to.be.revertedWith(
          "CollateralPoolConfig/invalid-close-factor-bps"
        );
      });
    });
    context("when close factor bps is 0", () => {
      it("should be revert", async () => {
        await expect(collateralPoolConfig.setCloseFactorBps(COLLATERAL_POOL_ID, BigNumber.from(0))).to.be.revertedWith(
          "CollateralPoolConfig/invalid-close-factor-bps"
        );
      });
    });
  ...
  describe("#setLiquidatorIncentiveBps", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setLiquidatorIncentiveBps(COLLATERAL_POOL_ID, LIQUIDATOR_INCENTIVE_BPS)).to.be.revertedWith(
          "!ownerRole"
        );
      });
    });
    context("when liquidator incentive bps is more than 20000", () => {
      it("should be revert", async () => {
        await expect(collateralPoolConfig.setLiquidatorIncentiveBps(COLLATERAL_POOL_ID, BigNumber.from(20000))).to.be.revertedWith(
    ...
    context("when liquidator incentive bps is less than 10000", () => {
      it("should be revert", async () => {
        await expect(collateralPoolConfig.setLiquidatorIncentiveBps(COLLATERAL_POOL_ID, BigNumber.from(9000))).to.be.revertedWith(
          "CollateralPoolConfig/invalid-liquidator-incentive-bps"
        );
      });
    });
  ...
  describe("#setTreasuryFeesBps", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setTreasuryFeesBps(COLLATERAL_POOL_ID, TREASURY_FEE_BPS)).to.be.revertedWith("!ownerRole");
      });
    });
    context("when treasury fee bps is more than 9000", () => {
      it("should be revert", async () => {
        await expect(collateralPoolConfig.setTreasuryFeesBps(COLLATERAL_POOL_ID, BigNumber.from(20000))).to.be.revertedWith(
          "CollateralPoolConfig/invalid-treasury-fees-bps"
        );
      });
    });
    context("when parameters are valid", () => {
  ...
  describe("#setTotalDebtShare", () => {
    context("when the caller is not the Bookkeeper Role", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setTotalDebtShare(COLLATERAL_POOL_ID, WeiPerRay)).to.be.revertedWith("!bookKeeperRole");
      });
    });
    context("when parameters are valid", () => {
      it("should success", async () => {
  ...
  describe("#setDebtAccumulatedRate", () => {
    context("when the caller is not the Bookkeeper Role", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setDebtAccumulatedRate(COLLATERAL_POOL_ID, WeiPerRay)).to.be.revertedWith("!bookKeeperRole");
      });
    });
    context("zero debt accumulated rate", () => {
      it("should be revert", async () => {
        await expect(collateralPoolConfigAsAlice.setDebtAccumulatedRate(COLLATERAL_POOL_ID, 0)).to.be.revertedWith(
          "CollateralPoolConfig/invalid-debt-accumulated-rate"
        );
      });
    });
    context("debt accumulated rate less then ray", () => {
      it("should be revert", async () => {
        await expect(collateralPoolConfigAsAlice.setDebtAccumulatedRate(COLLATERAL_POOL_ID, WeiPerRay.sub(1))).to.be.revertedWith(
          "CollateralPoolConfig/invalid-debt-accumulated-rate"
        );
      });
    });
  ...
  describe("#setStrategy", () => {
    context("when the caller is not the owner", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.setStrategy(COLLATERAL_POOL_ID, AddressZero)).to.be.revertedWith("!ownerRole");
      });
    });
  ...
  describe("#updateLastAccumulationTime", () => {
    context("when the caller is not the StabilityFeeCollector role", () => {
      it("should be revert", async () => {
        ...
        await expect(collateralPoolConfigAsAlice.updateLastAccumulationTime(COLLATERAL_POOL_ID)).to.be.revertedWith("!stabilityFeeCollectorRole");
      });
    });
    context("when parameters are valid", () => {
      it("should success", async () => {
  ...
  describe("#getLiquidatorIncentiveBps", () => {
    context("when parameters are valid", () => {
      it("should success", async () => {
        ...
        expect(await collateralPoolConfig.getLiquidatorIncentiveBps(COLLATERAL_POOL_ID)).to.be.equal(LIQUIDATOR_INCENTIVE_BPS);
      });
    });
  });
