...
describe("SlidingWindowDexOracle", () => {
  ...
  beforeEach(async () => {
    const SlidingWindowDexOracle = await ethers.getContractFactory("MockSlidingWindowDexOracle");
    slidingWindowDexOracle = await SlidingWindowDexOracle.deploy();
  ...
  describe("#getPrice()", async () => {
    ...
    context("historical observation exists", async () => {
      it("should store the cumulative price", async () => {
        ...
        await time.increase(300);
        const t = await slidingWindowDexOracle.update(mockedToken.address, mockedUSD.address);
        ...
        const price = await slidingWindowDexOracle.getPrice(mockedToken.address, mockedUSD.address);
      ...
      context("different decimals", async () => {
        it("return correct price", async () => {
          ...
          const price0 = await slidingWindowDexOracle.getPrice(mockedToken.address, mockedUSD.address);
          const price1 = await slidingWindowDexOracle.getPrice(mockedUSD.address, mockedToken.address);
