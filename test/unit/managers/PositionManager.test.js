...
describe("PositionManager", () => {
  ...
  beforeEach(async () => {
    ...
    mockedBookKeeper.whitelist.returns();
    ...
    context("when price is not healthy", () => {
      it("should revert", async () => {
        ...
        mockedBookKeeper.adjustPosition
          .whenCalledWith(formatBytes32String("WXDC"), positionAddress, positionAddress, positionAddress, parseEther("1"), parseEther("50"))
          .returns();
  ...
  describe("#moveCollateral(uint256,address,uint256,bytes)", () => {
    ...
    context("when price is not healthy", () => {
      it("should revert", async () => {
        ...
        mockedBookKeeper.moveCollateral.whenCalledWith(formatBytes32String("WXDC"), positionAddress, BobAddress, parseEther("1")).returns();
  ...
  describe("#exportPosition()", () => {
    ...
    context("when Alice wants to export her own position to her own address", async () => {
      it("should be able to call exportPosition()", async () => {
        ...
        mockedBookKeeper.positions.whenCalledWith(formatBytes32String("WXDC"), positionAddress).returns([WeiPerWad.mul(2), WeiPerWad.mul(1)]);
        mockedBookKeeper.movePosition
          .whenCalledWith(formatBytes32String("WXDC"), positionAddress, AliceAddress, WeiPerWad.mul(2), WeiPerWad.mul(1))
          .returns();
    ...
    context("when Alice wants Bob to export her position to Bob's address", async () => {
      it("should be able to call exportPosition()", async () => {
        ...
        mockedBookKeeper.positions.whenCalledWith(formatBytes32String("WXDC"), positionAddress).returns([WeiPerWad.mul(2), WeiPerWad.mul(1)]);
        mockedBookKeeper.movePosition
          .whenCalledWith(formatBytes32String("WXDC"), positionAddress, BobAddress, WeiPerWad.mul(2), WeiPerWad.mul(1))
          .returns();
  ...
  describe("#importPosition()", () => {
    ...
    context("when Alice wants to import her own position from her address", async () => {
      it("should be able to call importPosition()", async () => {
        ...
        mockedBookKeeper.positions.whenCalledWith(formatBytes32String("WXDC"), AliceAddress).returns([WeiPerWad.mul(2), WeiPerWad.mul(1)]);
        mockedBookKeeper.movePosition
          .whenCalledWith(formatBytes32String("WXDC"), AliceAddress, positionAddress, WeiPerWad.mul(2), WeiPerWad.mul(1))
          .returns();
    ...
    context("when Alice wants Bob to import her position from Bob's address", async () => {
      it("should be able to call importPosition()", async () => {
        ...
        mockedBookKeeper.positions.whenCalledWith(formatBytes32String("WXDC"), BobAddress).returns([WeiPerWad.mul(2), WeiPerWad.mul(1)]);
        mockedBookKeeper.movePosition
          .whenCalledWith(formatBytes32String("WXDC"), BobAddress, positionAddress, WeiPerWad.mul(2), WeiPerWad.mul(1))
          .returns();
  ...
  describe("#movePosition()", () => {
    ...
    context("when Alice wants to move her position#1 to her position#2", async () => {
      it("should be able to call movePosition()", async () => {
        ...
        mockedBookKeeper.positions.whenCalledWith(formatBytes32String("WXDC"), position1Address).returns([WeiPerWad.mul(2), WeiPerWad.mul(1)]);
        mockedBookKeeper.movePosition
          .whenCalledWith(formatBytes32String("WXDC"), position1Address, position2Address, WeiPerWad.mul(2), WeiPerWad.mul(1))
          .returns();
  ...
  describe("#redeemLockedCollateral()", () => {
    ...
    context("when parameters are valid", () => {
      it("should be able to redeemLockedCollateral", async () => {
        await positionManager.open(formatBytes32String("WXDC"), AliceAddress);
        const position1Address = await positionManager.positions(1);
        mockedShowStopper.redeemLockedCollateral.whenCalledWith(formatBytes32String("WXDC"), position1Address, AliceAddress, "0x").returns();
