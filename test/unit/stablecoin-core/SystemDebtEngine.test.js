  ...
  describe("#withdrawCollateralSurplus", () => {
    context("when the caller is not the owner", async () => {
      it("should revert", async () => {
        ...
        await expect(
          systemDebtEngineAsAlice.withdrawCollateralSurplus(formatBytes32String("WNATIVE"), DeployerAddress, UnitHelpers.WeiPerWad)
    ...
    context("when the caller is the owner", async () => {
      it("should be able to call withdrawCollateralSurplus", async () => {
        ...
        mockedBookKeeper.moveCollateral
          .whenCalledWith(formatBytes32String("WNATIVE"), systemDebtEngine.address, DeployerAddress, UnitHelpers.WeiPerWad)
          .returns();

        await systemDebtEngine.withdrawCollateralSurplus(formatBytes32String("WNATIVE"), DeployerAddress, UnitHelpers.WeiPerWad);
      });
    });
