...
describe("FathomStablecoin", () => {
  ...
  beforeEach(async () => {
    ...
    fathomStablecoinAsAlice = fathomStablecoin.connect(provider.getSigner(AliceAddress));
    ...
    context("when alice transfer to bob", () => {
      context("when alice doesn't have enough token", () => {
        it("should be revert", async () => {
          await expect(fathomStablecoinAsAlice.transferFrom(AliceAddress, BobAddress, WeiPerWad.mul(100))).to.be.revertedWith(
      ...
      context("amount is zero", () => {
        it("should be revert", async () => {
          await expect(fathomStablecoinAsAlice.transferFrom(AliceAddress, BobAddress, 0)).to.be.revertedWith("FathomStablecoin/zero-amount");
        });
      });
      context("destination is zero", () => {
        it("should revert", async () => {
          await expect(fathomStablecoinAsAlice.transferFrom(AliceAddress, AddressZero, 20)).to.be.revertedWith("FathomStablecoin/zero-destination");
        });
      });
      context("when alice has enough token", () => {
        context("when the caller is not the owner", async () => {
          it("should revert", async () => {
            ...
            await expect(fathomStablecoin.transferFrom(AliceAddress, BobAddress, WeiPerWad.mul(100))).to.be.revertedWith(
  ...
  context("#decreaseAllowance", () => {
    context("decrease below zero", () => {
      it("should revert", async () => {
        ...
        await expect(fathomStablecoinAsAlice.decreaseAllowance(DeployerAddress, WeiPerWad.add(1))).to.be.revertedWith(
          "FathomStablecoin/decreased-allowance-below-zero"
        );
  ...
  context("#burn", () => {
    context("when alice doesn't have enough token", () => {
      it("should be revert", async () => {
        await expect(fathomStablecoinAsAlice.burn(AliceAddress, WeiPerWad.mul(100))).to.be.revertedWith("FathomStablecoin/insufficient-balance");
      });
    });
    context("when alice has enough token", () => {
      context("when the caller is not the owner", async () => {
        it("should revert", async () => {
          await fathomStablecoin.grantRole(await fathomStablecoin.MINTER_ROLE(), DeployerAddress);
          await fathomStablecoin.mint(AliceAddress, WeiPerWad.mul(100));
          await expect(fathomStablecoin.burn(AliceAddress, WeiPerWad.mul(100))).to.be.revertedWith("FathomStablecoin/insufficient-allowance");
        });
        context("when Alice set allowance", () => {
          context("when allowance is not enough", () => {
            it("should revert", async () => {
