...
const walletArbitrauger = new hre.ethers.Wallet(privateKey3,provider);
...
async function main() {
    ...
    await fathomToken.mint(DeployerAddress, parseEther("10000"))
    ...
    await fairLaunch.addPool(1, USDT.address, true)
...
//    await fathomStablecoin.mint(DeployerAddress, parseEther("50000"))
    ...
    const authTokenAdapter = await AuthTokenAdapter.deploy(bookKeeper.address,COLLATERAL_POOL_ID,USDT.address);
    console.log("AuthTokenAdapter deployed to :", authTokenAdapter.address);

    const StableSwapModule = await hre.ethers.getContractFactory("StableSwapModule");
    const stableSwapModule = await StableSwapModule.deploy(authTokenAdapter.address,stablecoinAdapter.address, systemDebtEngine.address);
    console.log("StableSwapModule deployed to :", stableSwapModule.address);
    ...
    initialMinting(USDT,WXDC,bookKeeper,fathomStablecoin)
    ...
    fs.writeFileSync('./scripts/e_stableSwapScenario/cupcakes/0_deployment.json', data);
}

async function initialMinting(USDT,WXDC,bookKeeper,fathomStablecoin){

    //Mint 1 million USDT to deployer
    await USDT.mint(DeployerAddress, parseEther("10000"))

    //Mint 1 million WXDC to deployer
    await WXDC.mint(DeployerAddress, parseEther("10000"))

    await USDT.mint(walletAlice.address, parseEther("10000"))

    //Mint unbacked stablecoin to Alice : 500
    // await bookKeeper.mintUnbackedStablecoin(DeployerAddress, walletAlice.address, WeiPerRad.mul(1100))
    // await fathomStablecoin.mint(walletAlice.address, parseEther("1100"))

    //Mint 500 USDT to Arbitrauger
    await USDT.mint(walletArbitrauger.address, parseEther("1000"))
} 
