...
let rawdata2 = fs.readFileSync('./scripts/c_positionMoveScenario/cupcakes/2_proxyWalletAddresses.json');
...
async function main() {
    ...
    // src position and dst position need to be owned by the msg.sender(Usr's EOA)
    // onlyOwnerAllowed(_sourceId)
    // onlyOwnerAllowed(_destinationId)
    // also whenNotPaused
    await movePosition(proxyWalletAsAlice, firstPositionId, secondPositionId);
    ...
    async function movePosition(proxyWallet, sourcePositionId, destinationPositionId) {

        await printCollateralAndDebtShare(firstPositionId);
        await printCollateralAndDebtShare(secondPositionId);

        // https://github.com/ethers-io/ethers.js/issues/478
        let movePositionAbi = [
            "function movePosition(address _manager, uint256 _source, uint256 _destination)"
        ];
        let movePositionIFace = new hre.ethers.utils.Interface(movePositionAbi);
        let openPositionCall = movePositionIFace.encodeFunctionData("movePosition", [
            positionManager.address,
            sourcePositionId,
            destinationPositionId
        ]);

        const movePositionTx = await proxyWallet.execute2(fathomStablecoinProxyActions.address, openPositionCall);
