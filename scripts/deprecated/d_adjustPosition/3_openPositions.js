...
async function main() {
    ...
    // We opening 5 USXD at a time with 10 WXDC as collateral.
    // since adjusting Position need more space in safetyBuffer
    ...
    const alicePositionAddress1 = await positionManager.positions(alicePositionId_1);
    console.log("Position 1 opened for Alice");

    // Travel to the future
    await hre.ethers.provider.send("evm_increaseTime", [BigNumber.from("31536000").toNumber()])
    await hre.ethers.provider.send("evm_mine", [])

    // 3. Open one more position as Alice.
    const alicePositionId_2 = await openPosition(AliceAddress, proxyWalletAsAlice, "Alice");
    const alicePositionAddress2 = await positionManager.positions(alicePositionId_2);
    console.log("Position 2 opened for Alice");
    ...
    fs.writeFileSync('./scripts/d_adjustPosition/cupcakes/3_positionHandlerAddresses.json', data);
