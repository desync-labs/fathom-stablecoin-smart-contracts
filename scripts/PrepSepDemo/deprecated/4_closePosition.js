...
async function main() {
    ...
    const WXDCAsBob = new hre.ethers.Contract(stablecoinAddress.WXDC, WXDCAbi, walletBob);

    // proxyWallet as signers
    const proxyWalletAsBob = new hre.ethers.Contract(proxyWalletBob, proxyWalletAbi, walletBob);

    // Approve stablecoin
    const stablecoinAsBob = new hre.ethers.Contract(fathomStablecoin.address, StableCoinArtifact.abi, walletBob);
    await stablecoinAsBob.approve(proxyWalletBob, WeiPerWad.mul(1));

    // Close position id 2
    await wipeAllAndUnlockToken(BobAddress, proxyWalletAsBob, 2);
