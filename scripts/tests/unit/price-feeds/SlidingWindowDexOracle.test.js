    ...
    describe("#getPrice()", async () => {
        ...
        context("historical observation exists", async () => {
            it("should store the cumulative price", async () => {
                ...
                let firstObservation = await slidingWindowDexOracle.getFirstObservationInWindow(mockedPair.address)
                let currentCumulative = await slidingWindowDexOracle.currentCumulativePrice(mockedPair.address)

                expectedPrice = BigNumber.from(mockedStablecoin.address).lt(BigNumber.from(mockedUSD.address)) 
