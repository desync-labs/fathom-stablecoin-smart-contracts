...
describe("FathomBridge", () => {
  ...
  beforeEach(async () => {
    ({ mockedAccessControlConfig, fathomBridge, mockedToken } = await loadFixture(loadFixtureHandler));
  });
  ...
  describe("#addToWhitelist", () => {
    ...
    context("when the caller is the owner but trying to add ZeroAddress", async () => {
      it("should revert", async () => {
        await mockedAccessControlConfig.mock.hasRole.returns(true);
        await expect(fathomBridge.addToWhitelist(ZeroAddress)).to.be.revertedWith("FathomBridge/zero-address");
      });
    });
  });
  ...
  describe("#removeFromWhitelist", () => {
    ...
    context("when the caller is the owner but trying to add ZeroAddress", async () => {
      it("should revert", async () => {
        await mockedAccessControlConfig.mock.hasRole.returns(true);
        await expect(fathomBridge.removeFromWhitelist(ZeroAddress)).to.be.revertedWith("FathomBridge/zero-address");
      });
    });
  });
  ...
  describe("#withdrawFees", () => {
    ...
    context("when the caller is the owner but withdraw fee to ZeroAddress", async () => {
      it("should revert", async () => {
        await mockedAccessControlConfig.mock.hasRole.returns(true);
        await expect(fathomBridge.withdrawFees(ZeroAddress)).to.be.revertedWith("FathomBridge/zero-address");
      });
    });
  });
  ...
  describe("#crossChainTransfer", () => {
    ...
    context("when the caller is the whitelisted but try to send to ZeroAddress", async () => {
      it("should revert", async () => {
        await fathomBridge.addToWhitelist(DeployerAddress);
        await expect(fathomBridge.crossChainTransfer(5522, ZeroAddress, WeiPerWad)).to.be.revertedWith("FathomBridge/zero-address");
      });
    });
  ...
  describe("#Cage", () => {
    ...
    context("when the caller is the owner", async () => {
      it("should work", async () => {
        await mockedAccessControlConfig.mock.hasRole.returns(true);
        await expect(fathomBridge.cage({ gasLimit: 1000000 })).to.be.emit(fathomBridge, "LogCage");
      });
    });
  });
  describe("#pause", () => {
