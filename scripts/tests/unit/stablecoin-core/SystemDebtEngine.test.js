...
describe("SystemDebtEngine", () => {
  ...
  beforeEach(async () => {
    ({ systemDebtEngine, systemDebtEngineAsAlice, mockedBookKeeper, mockedCollateralTokenAdapter, mockedAccessControlConfig } =
    ...
    context("when the caller is the owner", async () => {
      it("should be able to call withdrawCollateralSurplus", async () => {
        ...
        await mockedBookKeeper.mock.moveCollateral
          .withArgs(formatBytes32String("WXDC"), systemDebtEngine.address, DeployerAddress, UnitHelpers.WeiPerWad)
          .returns();
