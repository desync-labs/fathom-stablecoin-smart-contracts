...
describe("BookKeeper", () => {
  ...
  beforeEach(async () => {
    ({ bookKeeper, bookKeeperAsAlice, bookKeeperAsBob, mockedCollateralPoolConfig, mockedAccessControlConfig } =
  ...
  describe("#adjustPosition", () => {
    context("when bookkeeper does not live", () => {
      it("should be revert", async () => {
        ...
        await expect(
          bookKeeper.adjustPosition(COLLATERAL_POOL_ID, DeployerAddress, DeployerAddress, DeployerAddress, WeiPerWad, 0, { gasLimit: 1000000 })
        ...
        context("when call adjustPosition(draw)", () => {
          context("when alice call but bob is position owner", () => {
            it("should be revert", async () => {
              ...
              await expect(
                bookKeeperAsAlice.adjustPosition(COLLATERAL_POOL_ID, BobAddress, BobAddress, BobAddress, 0, WeiPerWad.mul(10), { gasLimit: 1000000 })
    ...
    context("when alice move position to bob", () => {
      context("when alice and bob don't allow anyone else to manage the position", () => {
        it("should be revert", async () => {
          ...
          await expect(
            bookKeeperAsAlice.movePosition(COLLATERAL_POOL_ID, AliceAddress, BobAddress, WeiPerWad.mul(5), WeiPerWad.mul(1), { gasLimit: 1000000 })
      ...
      context("when bob allow alice to manage a position", () => {
        context("when after moving alice position was not safe", () => {
          it("should be revert", async () => {
            ...
            await expect(
              bookKeeperAsAlice.movePosition(COLLATERAL_POOL_ID, AliceAddress, BobAddress, WeiPerWad.mul(10), WeiPerWad.mul(0), { gasLimit: 1000000 })
        ...
        context("when after moving bob position was not safe", () => {
          it("should be revert", async () => {
            ...
            await expect(
              bookKeeperAsAlice.movePosition(COLLATERAL_POOL_ID, AliceAddress, BobAddress, WeiPerWad.mul(0), WeiPerWad.mul(2), { gasLimit: 1000000 })
