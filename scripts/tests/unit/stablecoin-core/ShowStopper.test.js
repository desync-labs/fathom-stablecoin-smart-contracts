...
describe("ShowStopper", () => {
    ...
    context("when setting collateral pool is inactive", () => {
      ...
      context("pool is active", () => {
        it("should revert", async () => {
          ...
          await expect(showStopper.cagePool(formatBytes32String("XDC"))).to.be.revertedWith("ShowStopper/still-live");
        });
      });
      ...
      context("priceFeed's isPriceOk is false", () => {
        it("should be revert", async () => {
          ...
          await expect(showStopper.cagePool(formatBytes32String("XDC"))).to.be.revertedWith("ShowStopper/price-not-ok");
      ...
      context("pool is inactive", () => {
        ...
        context("when the caller has no access to the position", () => {
          it("should revert", async () => {
            ...
            await expect(
              showStopperAsAlice.redeemLockedCollateral(formatBytes32String("XDC"), DeployerAddress, DeployerAddress, "0x")
        ...
        context("and debtShare is 0 and lockedCollateral is 1 ray", () => {
          it("should be success", async () => {
            ...
            await mockedBookKeeper.mock.positions.withArgs(formatBytes32String("XDC"), DeployerAddress).returns(WeiPerRay, BigNumber.from("0"));
            await mockedBookKeeper.mock.confiscatePosition
              .withArgs(formatBytes32String("XDC"), DeployerAddress, DeployerAddress, mockedSystemDebtEngine.address, WeiPerRay.mul("-1"), 0)
              .returns();

            await expect(showStopper.redeemLockedCollateral(formatBytes32String("XDC"), DeployerAddress, DeployerAddress, "0x"))
        ...
        context("and debtShare is 0 and lockedCollateral is 1 ray, the caller is not the owner of the address but has access to", () => {
          it("should be success", async () => {
            ...
            await mockedBookKeeper.mock.positions.withArgs(formatBytes32String("XDC"), AliceAddress).returns(WeiPerRay, BigNumber.from("0"));
            await mockedBookKeeper.mock.confiscatePosition
              .withArgs(formatBytes32String("XDC"), AliceAddress, AliceAddress, mockedSystemDebtEngine.address, WeiPerRay.mul("-1"), 0)
              .returns();

            await expect(showStopper.redeemLockedCollateral(formatBytes32String("XDC"), AliceAddress, AliceAddress, "0x"))
              .to.emit(showStopper, "LogRedeemLockedCollateral")
  ...
  describe("#finalizeCashPrice", () => {
    context("when calculate cash price", () => {
      context("debt is 0", () => {
        it("should revert", async () => {
          await expect(showStopper.finalizeCashPrice(formatBytes32String("XDC"))).to.be.revertedWith("ShowStopper/debt-zero");
        });
      });
  ...
  describe("#redeemStablecoin", () => {
    context("when calculate cash", () => {
      context("cash price is not defined", () => {
        it("should revert", async () => {
          await expect(showStopper.redeemStablecoin(formatBytes32String("XDC"), WeiPerWad)).to.be.revertedWith(
