  ...
  describe("#whitelisting and decentralization", () => {
    ...
    context("when not whitelisted and decentralized ", () => {
      it("should be able to call flashLoan", async () => {
        ...
        await mockFathomStablecoin.mock.transferFrom
          .withArgs(
          ...
          .returns(true);
        await mockStablecoinAdapter.mock.deposit
          .withArgs(
            flashMintModule.address,
            //as fee rate is 0, the amount to be transferred is 10
            // WeiPerWad.mul(11),
        ...
        await mockBookKeeper.mock.mintUnbackedStablecoin.withArgs(flashMintModule.address, flashMintModule.address, WeiPerRad.mul(10)).returns();
        await mockStablecoinAdapter.mock.withdraw
          .withArgs(mockMyFashLoan.address, WeiPerWad.mul(10), ethers.utils.defaultAbiCoder.encode(["uint256"], [0]))
          .returns();
      ...
      it("should be able to call bookKeeper flashLoan", async () => {
        await flashMintModule.setDecentralizedStatesStatus(true);
        await flashMintModule.setMax(WeiPerWad.mul(100));
        await mockMyFashLoan.mock.onBookKeeperFlashLoan.returns(keccak256(toUtf8Bytes("BookKeeperFlashBorrower.onBookKeeperFlashLoan")));

        await mockBookKeeper.mock.mintUnbackedStablecoin.withArgs(flashMintModule.address, mockMyFashLoan.address, WeiPerRad.mul(10)).returns();
        await mockBookKeeper.mock.settleSystemBadDebt.withArgs(WeiPerRad.mul(10)).returns();
        await mockBookKeeper.mock.stablecoin.returns(0);
  ...
  describe("#bookKeeperFlashLoan", () => {
    ...
    context("when callback failed", () => {
      it("should be revert", async () => {
        ...
        await mockMyFashLoan.mock.onBookKeeperFlashLoan.returns(keccak256(toUtf8Bytes("")));
