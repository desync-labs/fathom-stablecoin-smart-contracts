...
describe("DelayFathomOraclePriceFeed", () => {
  ...
  //comments below is assuming that line 102 in PriceOracle.sol can be mocked with waffle
  //it would be better to ask Vitalii about this
  ...
  let accessControlConfig // <- later should be able to give deployer permission to call setPrice()
  // and update _priceWithSafetyMargin
  ...
  describe("MockDexPriceOracle.sol check before testing DelayFathomOraclePriceFeed", () => {
    context("call getPrice", async () => {
      it("should return 0 and block.timeStamp", async () => {

        await expect(
          MockDexPriceOracle.getPrice(), DeployerAddress, WeiPerWad)
        ).to.be.equal("!adapterRole")
      })
    })

    context("change mockPrice", async () => {
      context("call changePrice with ", () => {
