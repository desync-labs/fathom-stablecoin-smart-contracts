...
describe("CollateralTokenAdapter", () => {
  ...
  beforeEach(async () => {
    ({ collateralPoolConfig, accessControlConfig, WXDC, collateralTokenAdapter, wxdcAddr, bookKeeper } = await loadFixture(setup));
  });
  ...
  describe("#deposit", async () => {
    ...
    context("when all parameters are valid", async () => {
      it("should work", async () => {
        ...
        await collateralTokenAdapter.deposit(AliceAddress, 0, ethers.utils.defaultAbiCoder.encode(["address"], [AliceAddress]), {
          from: AliceAddress,
          gasLimit: 1000000,
        });
  ...
  describe("#cage/#uncage", async () => {
    ...
    context("when caller not owner role cage", async () => {
      context("when assumptions still valid", async () => {
        it("should revert", async () => {
          await expect(collateralTokenAdapter.cage({ from: AliceAddress, gasLimit: 1000000 })).to.be.revertedWith(
