    ...
    describe("#collect", () => {
        context("when call collect directly and call deposit", () => {
            it("should be success", async () => {
                await collateralPoolConfig.setStabilityFeeRate(pools.NATIVE, BigNumber.from("1000000005781378656804591713"), { gasLimit: 1000000 })
                ...
                await stabilityFeeCollector.collect(pools.NATIVE, { gasLimit: 1000000 })
                ...
                AssertHelpers.assertAlmostEqual(
                    (await bookKeeper.positions(pools.NATIVE, positionAddress)).debtShare.toString(),
                    "4564354645876384278"
                )
                AssertHelpers.assertAlmostEqual(
                    (await collateralPoolConfig.collateralPools(pools.NATIVE)).totalDebtShare.toString(),
                    "4564354645876384278"
                )
                AssertHelpers.assertAlmostEqual(
                ...
                AssertHelpers.assertAlmostEqual(
                    (await bookKeeper.positions(pools.NATIVE, positionAddress2)).debtShare.toString(),
                    "3803628871563653565"
                )
