...
module.exports = async function (deployer) {
  ...
  const promises = [
    ...
    bookKeeper.initialize(collateralPoolConfig.address, accessControlConfig.address, { gasLimit: 1000000 }),
    ...
    liquidationEngine.initialize(bookKeeper.address, systemDebtEngine.address, { gasLimit: 1500000 }),
    stablecoinAdapter.initialize(bookKeeper.address, fathomStablecoin.address, { gasLimit: 1000000 }),
    priceOracle.initialize(bookKeeper.address, { gasLimit: 1000000 }),
    showStopper.initialize(bookKeeper.address, { gasLimit: 1000000 }),
    positionManager.initialize(bookKeeper.address, showStopper.address, priceOracle.address, { gasLimit: 1000000 }),
    ...
    stabilityFeeCollector.initialize(bookKeeper.address, systemDebtEngine.address, { gaslimit: 4050000 }),
    proxyActionsStorage.initialize(fathomStablecoinProxyActions.address, bookKeeper.address, { gasLimit: 1000000 }),
    proxyWalletFactory.initialize(proxyActionsStorage.address, proxyWalletRegistry.address, { gasLimit: 1000000 }),
    proxyWalletRegistry.initialize(proxyWalletFactory.address, bookKeeper.address, { gasLimit: 1000000 }),
    flashMintModule.initialize(stablecoinAdapter.address, systemDebtEngine.address, { gasLimit: 1000000 }),
    ...
    collateralTokenAdapter.initialize(bookKeeper.address, pools.XDC, addresses.WXDC, proxyWalletFactory.address),
    ...
    stableSwapModuleWrapper.initialize(bookKeeper.address, stableSwapModule.address),
    simplePriceFeed.initialize(accessControlConfig.address),
    // slidingWindowDexOracle.initialize(addresses.DEXFactory, 1800, 15);
    fathomBridge.initialize(addresses.AsterizmInitializerLib, fathomStablecoin.address, accessControlConfig.address),
  ];
