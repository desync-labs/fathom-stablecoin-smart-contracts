...
let rawdata = fs.readFileSync('./scripts/k_withdrawColateral/cupcakes/0_deployment.json');
...
let proxyWalletsData = fs.readFileSync('./scripts/k_withdrawColateral/cupcakes/2_proxyWalletAddresses.json');
...
let positionsData = fs.readFileSync('./scripts/k_withdrawColateral/cupcakes/3_positionHandlerAddresses.json');
...
async function main() {
    ...
    // we need to unlock some collateral first
    // we don't want to use "unlockToken" function because it will move unlocked tokens to Alice
    // safety buffer will be derceased on this step
    await adjustPosition(proxyWalletAsAlice, 1,    WeiPerWad.mul(-1), 0, AliceAddress);

    console.log(WeiPerWad + " collateral was unlocked");
    await printLog(getPositions, bookKeeper);

    // withdraw unlocked tokens
    await withdrawToken(AliceAddress, proxyWalletAsAlice, WeiPerWad, 1);
...
async function withdrawToken(address, proxyWalletAs, amount, positionId) {
    let withdrawTokenAbi = [
        "function withdrawToken(address _manager, address _tokenAdapter, uint256 _positionId, uint256 _amount, bytes calldata _data)"
    ];
    
    const encodedAddress = hre.ethers.utils.defaultAbiCoder.encode(["address"], [address]);

    let withdrawTokenIFace = new hre.ethers.utils.Interface(withdrawTokenAbi);
    let withdrawTokenCall = withdrawTokenIFace.encodeFunctionData("withdrawToken", [
    ...
    const tx = await proxyWalletAs.execute2(fathomStablecoinProxyActions.address, withdrawTokenCall);
    console.log(amount + " collateral was withdrawn");
