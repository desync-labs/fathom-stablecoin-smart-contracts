...
contract PositionManager is IManager {
    ...
    /// @dev it is important to note that owner here is proxyWallet of a user. -- Sangjun
    ...
    constructor(address _bookKeeper, address _showStopper) public {
    ...
    function allowManagePosition(
        ...
        address _user,
        uint256 _ok
    ) external override    onlyOwnerAllowed(_positionId) {
    ...
    function allowMigratePosition(address _user, uint256 _ok) external override    {
    ...
    /// @param _user The user address that is owned this position // proxyWallet of a user
    function open(bytes32 _collateralPoolId, address _user) external override    returns (uint256) {
    ...
    function moveCollateral(
        ...
        address _adapter,
        bytes calldata _data
    ) external    onlyOwnerAllowed(_positionId) {
    ...
    function redeemLockedCollateral(
        ...
        bytes calldata _data
    ) external override    onlyOwnerAllowed(_posId) {
