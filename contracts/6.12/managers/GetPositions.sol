...
contract GetPositions {
    ...
    {
        if (_startIndex + (_offset) > PositionManager(_manager).lastPositionId())
            _offset = PositionManager(_manager).lastPositionId() - (_startIndex) + (1);
        ...
        for (uint256 _positionIndex = _startIndex; _positionIndex < _startIndex + (_offset); _positionIndex++) {
    ...
    {
        uint256 _collateralValue = _lockedCollateral * (_priceWithSafetyMargin);
        // _collateralValue = WAD * RAY    which is RAD
        uint256 _debtValue = _debtShare * (_debtAccumulatedRate);
        //_debtValue is WAD * RAY <- 1E+18 * 1E+27 = 1E+45
        // WAD * RAY = RAD
        //_debtValue is in RAD
        _safetyBuffer = _collateralValue >= _debtValue ? _collateralValue - (_debtValue) : 0;
    }
}
