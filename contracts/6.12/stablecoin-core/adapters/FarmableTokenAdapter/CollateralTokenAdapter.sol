...
/// It receives Fathom's collateralToken from users and deposit in Fathom's FairLaunch.
/// Hence, users will still earn FATHOM rewards while holding positions.
contract CollateralTokenAdapter is ICagable {
    ...
    function setTreasuryFeeBps(uint256 _treasuryFeeBps) external    {
    ...
    function setTreasuryAccount(address _treasuryAccount) external    {
        require(live == 1, "CollateralTokenAdapter/not-live");
        require(_treasuryAccount != address(0), "CollateralTokenAdapter/bad treasury account");
        treasuryAccount = _treasuryAccount;
    ...
    /// @dev Harvest and deposit received collateralToken to FairLaunch
    /// @param _positionAddress The address that holding states of the position
    /// @param _amount The collateralToken amount that being used as a collateral and to be deposited to FairLaunch
    ...
    function share(uint256 _amount) external view returns (uint256){
            uint256 _share = wdiv(mul(_amount, to18ConversionFactor), netAssetPerShare()); // [wad]
            return _share;
    }

    function biggerThanShare(address staker, uint256 _amount) external view returns (bool) {
            uint256 _share = wdiv(mul(_amount, to18ConversionFactor), netAssetPerShare()); // [wad]
            return stake[staker] >= _share;
    }

    /// @dev Harvest and withdraw collateralToken from FairLaunch
    /// @param _usr The address that holding states of the position
    /// @param _amount The collateralToken amount to be withdrawn from FairLaunch and return to user
    function withdraw(
        address _usr,
        uint256 _amount,
        bytes calldata /* _data */
    ...
    /// @dev EMERGENCY ONLY. Withdraw collateralToken from FairLaunch with invoking "_harvest"
    ...
    // Just meaning that the function below will be called when adjustPoisition function in PositionManager is called.
