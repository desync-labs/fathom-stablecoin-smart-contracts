...
/** @notice A contract which acts as a book keeper of the Fathom Stablecoin protocol. 
        It has the ability to move collateral token and stablecoin with in the accounting state variable. 
*/

contract BookKeeper is IBookKeeper, ICagable {
    ...
    function whitelist(address toBeWhitelistedAddress) external override {
    ...
    function blacklist(address toBeBlacklistedAddress) external override {
    ...
    // https://ethereum.stackexchange.com/questions/127357/accessing-storage-of-a-map-of-a-map
    // https://ethereum.stackexchange.com/questions/99565/how-to-read-public-variable-from-contract-using-ethers-js
    ...
    // --- Init ---
    constructor(address _collateralPoolConfig) public {
    ...
    */
    ...
    function confiscatePosition(
        ...
        address _stablecoinDebtor,    // <- system debt engine
        int256 _collateralAmount, // <- 0 value when the funtion is called because collateral is all liquidated but still debtShare is left in a liquidated position
        int256 _debtShare // <- negative value. Subtracting negative value on systemBadDebt, therefore, increases. 
    ) external override {
    ...
    */
    ...
    /** @dev Accrue stability fee or the mint interest rate.
            ...
            `debtAccumulatedRate` of a collateral pool is the exchange rate of the stablecoin minted from that pool (think of it like CollateralToken price from Lending Vault).
