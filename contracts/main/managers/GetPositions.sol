...
contract GetPositions is Initializable {
    ...
    function getAllPositionsAsc(
        address _manager,
        address _user
    ) external view returns (uint256[] memory _ids, address[] memory _positions, bytes32[] memory _collateralPools) {
    ...
    function _getPositionsAsc(
        ...
        uint256 _fromId,
        uint256 _size
    ) internal view returns (uint256[] memory _ids, address[] memory _positions, bytes32[] memory _collateralPools) {
    ...
    function getAllPositionsDesc(address _manager, address _user) external view returns (uint256[] memory, address[] memory, bytes32[] memory) {
        uint256 _count = PositionManager(_manager).ownerPositionCount(_user);
    ...
    function getPositionsDesc(
        ...
        uint256 _fromId,
        uint256 _size
    ) external view returns (uint256[] memory, address[] memory, bytes32[] memory) {
    ...
    function getPositionWithSafetyBuffer(
        ...
        uint256 _startIndex,
        uint256 _offset
    ) external view returns (address[] memory _positions, uint256[] memory _debtShares, uint256[] memory _safetyBuffers) {
        ...
        for (uint256 _positionIndex = _startIndex; _positionIndex < _startIndex.add(_offset); _positionIndex++) {
            ...
            (uint256 _lockedCollateral, uint256 _debtShare) = _bookKeeper.positions(_collateralPoolId, _positions[_resultIndex]);
